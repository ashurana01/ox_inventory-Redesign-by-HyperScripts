...
const InventorySlot: React.ForwardRefRenderFunction<HTMLDivElement, SlotProps> = (
  ...
  const refs = useMergeRefs([connectRef, ref]);
    ...
    <div
      ...
      onClick={handleClick}
      className={`inventory-slot ${item?.durability !== undefined ? 'has-durability' : ''}`}
      style={{
        filter:
        ...
        backgroundImage: isOver
          ...
          : (item?.name 
              ? `url(${getItemUrl(item as SlotWithItem)}), ${durabilityFill ? durabilityFill + ',' : ''} radial-gradient(circle, rgba(62, 69, 82, 0.8) 0%, rgba(20, 23, 30, 0.95) 100%)` 
              : `radial-gradient(circle, rgba(62, 69, 82, 0.8) 0%, rgba(20, 23, 30, 0.95) 100%)`),
        backgroundSize: isOver 
          ? (item?.name ? `100% 100%, 7vh, 100% 100%, 100% 100%` : `100% 100%, 100% 100%`)
          : (item?.name ? (durabilityFill ? `7vh, 100% 100%, 100% 100%` : `7vh, 100% 100%`) : `100% 100%`),
        border: isOver ? '1px dashed rgba(255,255,255,0.4)' : (item?.durability !== undefined ? `1px solid hsla(${(item.durability * 120) / 100}, 60%, 40%, 0.3)` : ''),
      ...
      {isSlotWithItem(item) && (
        ...
        >
          <div className="item-slot-header-wrapper">
            ...
            {/* NEW LAYOUT: Count on Left, Weight on Right */}
            <div className="item-slot-info-wrapper">
              <p className="item-count-display">{item.count && item.count > 0 ? `x${item.count.toLocaleString('en-us')}` : ''}</p>
